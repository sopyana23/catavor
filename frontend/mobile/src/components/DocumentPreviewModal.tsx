  ...
  useEffect(() => {
    ...
    const renderPage = async () => {
      try {
        ...
        // Cap pixelRatio to 1.75 on mobile to optimize GPU memory & prevent stutter
        ...
        if (err?.name !== 'RenderingCancelledException' && !isCancelled) {
          console.warn(`Mobile PDF page ${pageNum} render error:`, err);
          setPageLoading(false);
        }
      }
    };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', overflow: 'hidden' }}>
      ...
      >
        ...
        {pageLoading && <RefreshCw size={9} className="animate-spin" color="#38bdf8" />}
      </div>
...
const PdfPageSlot: React.FC<PdfPageSlotProps> = ({
  ...
  return (
    <div
      ref={slotRef}
      id={`mobile-pdf-page-slot-${pageNum}`}
      style={{
        position: 'relative',
        marginBottom: '1rem',
        borderRadius: '6px',
        boxShadow: '0 8px 30px rgba(0, 0, 0, 0.45), 0 0 0 1px rgba(255, 255, 255, 0.08)',
    ...
    >
      {isVisible ? (
        ...
        >
          <span style={{ fontSize: '0.78rem', fontWeight: 600 }}>Halaman {pageNum}</span>
          <span style={{ fontSize: '0.66rem', color: '#475569' }}>Gulir untuk melihat</span>
        </div>
      )}
    </div>
  );
...
export const DocumentPreviewModal: React.FC<DocumentPreviewModalProps> = React.memo(({
  ...
  // Lock background scroll on mobile to prevent document bubbling
  useEffect(() => {
    if (!isOpen) return;
    ...
    const loadDocumentData = async () => {
      ...
      try {
        ...
        if (isPDF) {
          ...
          // Calculate initial scale to fit available mobile viewport width cleanly
          try {
            ...
            const targetWidth = Math.min(screenW - 24, 640);
            const initialScale = Math.max(0.4, Math.min(2.0, +(targetWidth / defaultViewport.width).toFixed(2)));
        ...
        if (!active) return;
        console.warn('Mobile document preview failed to load:', err);
  ...
  // Handle hardware / escape back button
  useEffect(() => {
    if (!isOpen) return;
  ...
  const scrollToPage = useCallback((targetPage: number) => {
    const pageEl = document.getElementById(`mobile-pdf-page-slot-${targetPage}`);
    if (pageEl) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      console.warn('Direct blob download error, falling back:', e);
    }
  ...
  return (
    ...
    >
      {/* 1. Sleek, Non-wrapping Header Bar */}
      <div
        style={{
      ...
      >
        {/* Left: Document Icon & Neat Single-line Title & Metadata */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', minWidth: 0, flex: 1 }}>
          <div style={{
            ...
            border: `1px solid ${isPDF ? 'rgba(239, 68, 68, 0.28)' : 'rgba(56, 189, 248, 0.28)'}`,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            flexShrink: 0
          }}>
            <FileText size={17} color={isPDF ? '#ef4444' : '#38bdf8'} />
          </div>
          ...
          <div style={{ minWidth: 0, flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            {/* Clean Single-line Meta Row: No awkward word breaking */}
            <div style={{
              display: 'flex',
              alignItems: 'center',
              gap: '0.35rem',
              ...
              <span style={{ opacity: 0.35 }}>•</span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.15rem', color: '#34d399', fontWeight: 600 }}>
        ...
        {/* Right: Clean Action Buttons (Compact & Proportional) */}
          ...
          {isPDF && numPages > 1 && (
            <button
              ...
              style={{
                ...
                border: `1px solid ${viewMode === 'single' ? 'rgba(56, 189, 248, 0.35)' : 'rgba(255, 255, 255, 0.1)'}`,
      ...
      {/* 2. Main Scroll Area: Dedicated flex space that never gets overlapped by the footer */}
      <div
        ref={containerRef}
        style={{
          flex: 1,
      ...
      >
        {loading ? (
          <div style={{ textAlign: 'center', color: '#94a3b8', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.85rem' }}>
            <RefreshCw size={32} className="animate-spin" color="#38bdf8" />
            <span style={{ fontSize: '0.86rem', fontWeight: 600 }}>Menyiapkan pratinjau dokumen...</span>
            <span style={{ fontSize: '0.72rem', color: '#64748b' }}>Merender halaman secara hemat memori</span>
          </div>
        ) : error ? (
          <div style={{ textAlign: 'center', color: '#f87171', padding: '2rem', maxWidth: '340px' }}>
            <AlertCircle size={36} color="#ef4444" style={{ margin: '0 auto 0.75rem auto' }} />
            <p style={{ fontSize: '0.86rem', marginBottom: '1.25rem', lineHeight: 1.5 }}>{error}</p>
            <button
              type="button"
              onClick={handleDownload}
              style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
              <div
                ...
                style={{
                  ...
                  marginBottom: '1rem',
                  borderRadius: '6px',
                  boxShadow: '0 8px 30px rgba(0, 0, 0, 0.55), 0 0 0 1px rgba(255, 255, 255, 0.08)',
          ...
          <div style={{ textAlign: 'center', color: '#94a3b8', padding: '2.5rem 1rem' }}>
            ...
            <h4 style={{ margin: '0 0 0.5rem 0', color: '#ffffff', fontSize: '0.95rem' }}>Pratinjau Tidak Didukung</h4>
            <p style={{ margin: '0 0 1.25rem 0', fontSize: '0.78rem', color: '#94a3b8', maxWidth: '300px' }}>
              Format berkas ini dapat dibuka setelah diunduh ke perangkat Anda.
            </p>
            <button
              type="button"
              onClick={handleDownload}
      ...
      {/* 3. Anchored Bottom Dock: Fixed to bottom of modal, perfectly separated, NO OVERLAPPING! */}
      {isPDF && pdfDoc && !loading && !error && (
        <div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          {/* Navigation Group (Prev / Jump / Next) */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
            ...
            <form onSubmit={handleJumpSubmit} style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
              <input
                type="text"
                inputMode="numeric"
                value={jumpInput}
